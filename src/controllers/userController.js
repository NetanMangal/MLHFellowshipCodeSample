...
async function login(req, res, next) {
    ...
    try {
        ...
        if (errors) return res.status(400).send({ success: false, msg: errors });
        // 
        // if (!req.header('x-source')) return res.status(400).send({ success: false, msg: msg.user.login.sourceHeaderNotSent });
        // if (req.header('x-source') != 'mobile' && req.header('x-source') != 'web') return res.status(400).send({ success: false, msg: msg.user.login.invalidSourceHeader });
...
async function getProfile(req, res, next) {
    ...
    try {
        ...
        const alreadyPledged = await formModel.findOne({ "userId": req.userId });
        ...
        res.status(200).send({
            ...
            "gender": user.gender,
            "plegeDetails": alreadyPledged? alreadyPledged: {}
        });
    }
    catch (e) {
        console.log("Point 1 error " + e);
...
async function pledgeForm(req, res, next) {
    console.log('In Pledge Form');
    try {
        const donar = req.body;

        donar.userId = req.userId;

        const alreadyPledged = await formModel.findOne({ "userId": req.userId });
        // console.log(alreadyPledged)
        if (alreadyPledged) return res.status(400).send({ success: false, msg: msg.user.pledge.alreadyPledged });

        await formModel.create(donar);
        res.status(200).send({ success: true, message: msg.user.pledge.submitted })
...
async function feedback(req, res, next) {
    ...
    try {
        ...
        await feedbackForm.create(feedback);

        res.status(200).send({ success: true, message: msg.user.feedback.submitted })
...
module.exports.pledgeForm = pledgeForm;
